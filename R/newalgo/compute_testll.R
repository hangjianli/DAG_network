...
for(simID in simIDs){
  ...
  dfplot = df %>% dplyr::select(shdXmain, shdXbaseline, shdXff) %>% 
    gather(method, testll) %>% 
    mutate(testll = (testll - median(df$shdXbaseline))  / sqrt(prod(dim(X$X))))
  dfplot$method <- rep(c('BCD', 'Baseline', 'KGLasso'), each = 10)
  # dfplot$method <- rep(c('BCD', 'Baseline'), each = 10)
  # dfplot <- dfplot %>% filter(testll > -500)
  # dfplot$method <- rep(c('BCD', 'Baseline'), each = 10)
...
for(simID in simIDs){
  ...
  dfplot = df %>% dplyr::select(-shdXmain) %>% 
    ...
    gather(method, testll) %>% 
    mutate(testll = testll / sqrt(prod(dim(X$X))))
  # dfplot$method <- rep(c('BCD', 'Baseline'), each = 10)
  # dfplot <- dfplot %>% filter(testll < 100, testll > 0)
  plot_testll(dfplot, theta_type = simID)
  setwd("~/Documents/research/dag_network")  
}
